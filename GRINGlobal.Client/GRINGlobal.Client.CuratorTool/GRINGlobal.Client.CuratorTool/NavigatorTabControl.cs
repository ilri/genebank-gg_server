...
if (int.Parse(_usernameCooperatorID) == cooperatorID)
            {
        ...
        private void ux_TabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (tc.SelectedIndex > -1)
            {
                ...
                //if (tc.SelectedIndex == (tc.TabPages.Count - 1))
                {
                    ...
                    {
                        TabPage tp = BuildTabPageAndTreeView(newTabDialog.TabText, null);
...
//// Create the new tool strip menu item and bind it to the click event handler...
//ToolStripMenuItem tsmiNew = new ToolStripMenuItem(tp.Text, null, ux_tab_tsmi_NavigatorShowTabsItem_Click, tp.Text);
...
//lastTabName = "";
        ...
        private void ux_tab_cms_NavigatorNewTab_Click(object sender, EventArgs e)
        {
            ...
            if (newTabDialog.ShowDialog(this) == DialogResult.OK)
            {
                ...
                TabPage tpNew = BuildTabPageAndTreeView(newTabDialog.TabText, null);
//TreeView tvNew = (TreeView)tpNew.Controls[tpNew.Name + "TreeView"];
//// Build the root node...
                ...
                foreach (string idType in idTypes)
                {
                    ...
                    foreach (string formatToken in formatTokens)
                    {
                            ...
                            formatToken.Contains("}"))
                        {
                            ...
                            if (ds.Tables[dataviewAndField[0]].Columns.Contains(dataviewAndField[1]))
                            {
...
//}
        ...
        private TabPage BuildTabPageAndTreeView(string tabPageName, DataTable treeviewItems)
        {
                ...
                treeviewItems.Rows.Count > 0)
            {
                ...
                sortedTreeviewItems.DefaultView.Sort = "SORT_ORDER ASC";
...
//        }
        ...
        private List<KeyValuePair<string, int>> BuildPKeyList(TreeNode treeNode, string dvParamList, bool includeSubfolders)
        {
...
//if (string.IsNullOrEmpty(resolveTo)) dvParamList = resolveTo;
                        resultItemList.AddRange(ResolveItemList(queryString, resolveTo, dvParamList, int.Parse(ux_numericupdownMaxRowsReturned.Value.ToString())));
                    }
                    ...
                    foreach (TreeNode ftn in treeNode.Nodes)
                    {
                        ...
                        if (isFolder(ftn))
                        {
                            ...
                            if (includeSubfolders) resultItemList.AddRange(BuildPKeyList(ftn, dvParamList, includeSubfolders));
                        }
...
string resolveTo = GetTreeviewNodeProperty("DYNAMIC_FOLDER_RESOLVE_TO", treeNode.Parent, false, "default");
                            resultItemList.AddRange(ResolveItemList(properties[2], resolveTo, dvParamList, int.Parse(ux_numericupdownMaxRowsReturned.Value.ToString())));
                        }
                    }
                }
            }
...
string resolveTo = GetTreeviewNodeProperty("DYNAMIC_FOLDER_RESOLVE_TO", treeNode.Parent, false, "default");
        ...
        private List<KeyValuePair<string, int>> ResolveItemList(string query, string resolveTo, string dvParamList, int limit)
        {
            List<KeyValuePair<string, int>> resultItemList = new List<KeyValuePair<string, int>>();
...
//if (dvParamList.Contains(":inventoryid"))
//{
//    searchResultType = "inventory";
//    searchParameter = ":inventoryid";
//}
//else if (dvParamList.Contains(":accessionid"))
//{
//    searchResultType = "accession";
//    searchParameter = ":accessionid";
//}
...
//((TreeView)sender).Focus();
        ...
        private void treeView_DragOver(object sender, DragEventArgs e)
        {
            ...
            // Is this a query criterial from the Search Tool...
