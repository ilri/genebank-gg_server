...
if (string.IsNullOrEmpty(ux_statusListMessage.Text)) ux_statusListMessage.Text = "";
...
ux_statusListMessage.Tag = ux_statusListMessage.Text;
...
else
{
    if (ux_tabcontrolSTDataviews.SelectedTab.Tag.GetType() == typeof(GRINGlobal.Client.Common.DataviewProperties))
    {
        ...
        private void LoadUserSettings()
        {
            ...
            // Get the setting for DynamicSECheckbox...
            tempBool = ux_checkboxDynamicSE.Checked;
            if (bool.TryParse(_sharedUtils.GetUserSetting(ux_checkboxDynamicSE.Name, "Checked", tempBool.ToString()), out tempBool)) {
                ux_checkboxDynamicSE.Checked = tempBool;
            }
        ...
        private void SetGeneralUserSettings()
        {
            ...
            _sharedUtils.SaveUserSetting(ux_checkboxDynamicSE.Name, "Checked", ux_checkboxDynamicSE.Checked.ToString());
...
//int pageSize = 100;
            ...
            if (ux_radiobuttonBasicReturnDataview.Checked) {     //KFE
                filter = "ID IS NOT NULL";
                //searchResultType = ((GRINGlobal.Client.Common.DataviewProperties)ux_tabcontrolSTDataviews.SelectedTab.Tag).DataviewName; //KFE
                //searchResultType = searchResultType.Replace("get_", "");
                searchResultType = currentTable();
            }
            else if (ux_radiobuttonBasicReturnCooperators.Checked)
            {
            ...
            try {
                ...
                if (ux_checkboxDynamicSE.Checked) {
                    options = "SearchEngineType='dynamic'";
                } else {
                    options = "SearchEngineType='static'";
                }
                if (ux_radiobuttonBasicMatchList.Checked) {
                    options += ";OrMultipleLines=true";
                }
                dsBasicQuerySearchResults = _sharedUtils.SearchWebService(searchCriteria, ux_checkboxIgnoreCase.Checked, ux_radiobuttonBasicMatchAll.Checked, tablesToSearch, searchResultType, 0, Int32.Parse(ux_numericupdownBasicMaxRecords.Value.ToString()), options);
                ...
                if (dsBasicQuerySearchResults != null && dsBasicQuerySearchResults.Tables.Contains("SearchResult")) {
                    ...
                    if (basicQuerySearchResults.Length == 0) {
                        ...
                        //GRINGlobal.Client.Common.GGMessageBox ggMessageBox = new GRINGlobal.Client.Common.GGMessageBox("Found at least {0} 'possible' {1} hits in the database.  Continue to retrieve data?", "Query Results", MessageBoxButtons.OKCancel, MessageBoxDefaultButton.Button1);  //KFE
                        //ggMessageBox.Name = "GRINGlobalClientSearchTool_ux_buttonDoBasicQueryMessage1";
                        GRINGlobal.Client.Common.GGMessageBox ggMessageBox = new GRINGlobal.Client.Common.GGMessageBox("Found {0} matches in the database.  Continue to retrieve data?", "Query Results", MessageBoxButtons.OKCancel, MessageBoxDefaultButton.Button1);  //KFE
                        ggMessageBox.Name = "GRINGlobalClientSearchTool_ux_buttonDoBasicQueryMessage6";
                        _sharedUtils.UpdateControls(ggMessageBox.Controls, ggMessageBox.Name);
                        if (ggMessageBox.MessageText.Contains("{0}")) ggMessageBox.MessageText = string.Format(ggMessageBox.MessageText, basicQuerySearchResults.Length.ToString() + " " + searchResultType); //KFE
                        userResponse = ggMessageBox.ShowDialog();
                    }
                    ux_statusListMessage.Text = "from " +basicQuerySearchResults.Length.ToString() + " " + searchResultType;
...
//ux_statusRightMessage.Text = "   Press the escape key (esc) to cancel query   ";
                ...
                while (pageStart < groupList.Length && !escapeKeyPressed) {
                    pkids = ""; // KFE any id type
                    for (int i = pageStart; i < pageStop; i++) {
                        pkids += groupList[i]["ID"].ToString() + ",";   //KFE changed default list from acids to generic pkids
                    }
                    // Update the paging indexes...
                    pageStart = pageStop;
                    pageStop = Math.Min((pageStart + pageSize), groupList.Length);
                    // Build the param string and get the data...
                    string pkeyCollection = ":" + searchResultType + "_id=" + pkids.TrimEnd(','); // + ":accessionid=; :inventoryid=; :cooperatorid=; :geographyid=; :cropid=; :taxonomygenusid=";
                    ...
                    DataSet pagedDataSet = _sharedUtils.GetWebServiceData2(dataviewName, pkeyCollection, 0, Int32.Parse(ux_numericupdownBasicMaxRecords.Value.ToString()));
...
ggMessageBox.ShowDialog();
        ...
        private void ux_optionControlChanged(object sender, EventArgs e)
        {
            ...
            if (ux_radiobuttonBasicReturnDataview.Checked) {
                //KFE attempt to get the resolve table from the dataview name
                //searchResultType = ((GRINGlobal.Client.Common.DataviewProperties)ux_tabcontrolSTDataviews.SelectedTab.Tag).DataviewName; //KFE
                //searchResultType = searchResultType.Replace("get_", "");
                searchResultType = currentTable();
            }
...
else
{
    ...
    else
    {
        ...
        private void ux_textboxBasicQueryText_MouseDown(object sender, MouseEventArgs e) {
            ...
            if (e.Button == MouseButtons.Left) {
                ...
                if (pointerCharIndex > txtBoxSelectionStart + 1 &&
                    ...
                    searchCriteria += "SearchCriteria=" + txtBox.Text.Substring(txtBoxSelectionStart, txtBoxSelectionLength);
        ...
        private string currentTable() {
            ...
            if (dt == null || dt.Rows.Count < 1) return "";
            foreach (DataGridViewCell dgvc in ux_datagridviewSearchCriteria.Rows[0].Cells) {
                if (dt.Columns[dgvc.ColumnIndex].ExtendedProperties.Contains("table_name") &&
                                !string.IsNullOrEmpty(dt.Columns[dgvc.ColumnIndex].ExtendedProperties["table_name"].ToString()) &&
                                dt.Columns[dgvc.ColumnIndex].ExtendedProperties.Contains("table_field_name") &&
                                !string.IsNullOrEmpty(dt.Columns[dgvc.ColumnIndex].ExtendedProperties["table_field_name"].ToString())
                    ) {
                    string fieldname = dt.Columns[dgvc.ColumnIndex].ExtendedProperties["table_field_name"].ToString().ToLower().Trim();
                    string tablename = dt.Columns[dgvc.ColumnIndex].ExtendedProperties["table_name"].ToString().ToLower().Trim();
                    return tablename;
                }
            }
            return "";
