...
loginDialog.StartPosition = FormStartPosition.CenterScreen;
        ...
        public DataSet GetWebServiceData2(string dataviewName, string delimitedParameterList, int offset, int limit) {
            DataSet results = new DataSet();
            if (IsConnected) {
                results = _webServices.GetData2(dataviewName.ToLower(), delimitedParameterList, offset, limit);
            }
            return results;
        }

        public string DataviewName2PKNameWebService(string dataviewName) {
            string results = "";
            if (IsConnected) {
                results = _webServices.DataviewName2PKName(dataviewName.ToLower());
            }
            return results;
        }
