...
namespace GrinGlobal.InstallHelper {
    class Utility {
        ...
        public static void SetDatabaseInstanceName(string instanceName) {
            Registry.SetValue(RootRegistryKey, "DatabaseInstanceName", instanceName);
        }
        public static void SetDatabaseWindowsAuth(bool windowsAuth) {
            Registry.SetValue(RootRegistryKey, "DatabaseWindowsAuth", windowsAuth ? "1" : "0");
        }
        ...
        public static bool IsLastGRINGlobalApplication(string displayName) {
            if (string.IsNullOrEmpty(displayName)) {
                return false;
            }
            ...
            foreach (string app in apps) {
                if (app.ToLower().StartsWith("grin-global")) {
                    if (app.ToLower() != displayName.ToLower()) {
                        return false;
                    }
                }
            }
        ...
        public static string ExtractCabFile(string cabFile, string destFolder) {
            ...
            ProcessStartInfo psi = new ProcessStartInfo((Utility.GetTargetDirectory(null, null, "ConfigurationWizard") + @"\gguac.exe").Replace(@"\\", @"\"), " /wait /hide expand.exe " + cabFile + " -F:* " + destFolder);
