...
// Temp: Similar to cart.cs, but if Favorite uses new table, say web_user_item_list, or need to deal with folder/subfolder etc, then could differ a lot from cart.cs later. So seperate them now

namespace GrinGlobal.Web {
    public class Favorite {
        /// <summary>
        /// Gets the favorite for the current user, loading from session or database as needed.  If one does not exist, a new, empty one will be created.
        /// </summary>
        public static Favorite Current {
            get {
                ...
                if (HttpContext.Current != null && HttpContext.Current.Session != null) {
                    favorite = HttpContext.Current.Session["favorite"] as Favorite;
                    if (favorite != null) {
                        return favorite;
                    } else {
                        ...
                        HttpContext.Current.Session["favorite"] = favorite;
                    }
                }
                return favorite;
            }
        ...
        /// <summary>
        /// Gets the array of accessions the current user has in their favorite.  Alter this array by calling AddAccession and RemoveAccession.
        /// </summary>
        ...
        /// <summary>
        /// Adds given ids to the Accessions array.  Returns # of new items added (as opposed to current items' quantities being updated)
        /// </summary>
        /// <param name="id"></param>
        ...
        public DataTable ListDistributionTypes(int accessionID) {
            using (SecureData sd = UserManager.GetSecureData(true)) {
                return sd.GetData("web_lookup_distribution_type_fav", ":accessionid=" + accessionID + ";:langid=" + sd.LanguageID, 0, 0).Tables["web_lookup_distribution_type_fav"];
...
//where
//	i.accession_id = :accessionid order by value desc
...
//                }
        ...
        /// <summary>
        /// Populates the Favorite object with values from the database.
        /// </summary>
        public void Fill() {
...
where
    web_user_cart_id in
    (select web_user_cart_id from web_user_cart where cart_type_code = 'favorites' and expiration_date < :now)
...
delete from 
    web_user_cart 
where 
    cart_type_code = 'favorites' and expiration_date < :now
", new DataParameters(":now", now, DbType.DateTime2));
     ...
     ":now", DateTime.UtcNow, DbType.DateTime2
        ...
        public void Save(int daysUntilExpiration) {
            ...
            if (UserManager.IsAnonymousUser(UserManager.GetUserName())) {
                ...
                if (HttpContext.Current != null && HttpContext.Current.Session != null) {
                    HttpContext.Current.Session["favorite"] = this;
                }
                _changed = false;
                return;
            }
            ...
            using (SecureData sd = new SecureData(false, UserManager.GetLoginToken(true))) {
                ...
                using (DataManager dm = sd.BeginProcessing(true)) {
                    ...
                    int wuserid = sd.WebUserID;
                    string cart_type_code = "favorites";  // TODO: retrive the correct code value lang when it is there
